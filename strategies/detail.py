...
def _extract_from_detail_page(html: str, url: str) -> dict:
    ...
    # ── upc / gtin / ean ────────────────────────────────────────────────────
    # Extract all barcode identifiers.  `upc` is the primary field (coerced
    # from whatever source).  `gtin` and `ean` capture the raw JSON-LD values
    # so they appear as separate columns in the spreadsheet.
    try:
        upc = ""
        gtin = ""
        ean = ""
        ...
        for gtin_key in ["gtin13", "gtin12", "gtin", "gtin8"]:
            ...
            coerced = _coerce_upc(raw)
            if coerced:
                upc = coerced
                # Store the raw value in the appropriate field
                if gtin_key == "gtin13" and len(raw.strip("0")) > 0:
                    ean = raw    # 13-digit GTIN is an EAN-13
                gtin = raw       # always store in gtin
                logger.info(f"[Strategy 2] UPC via JSON-LD ({gtin_key}) | upc={upc} gtin={gtin} ean={ean}")
                break
        ...
        if not upc:
            for meta_attrs in [
                ...
                if coerced:
                    upc = coerced
                    attr_name = list(meta_attrs.values())[0]
                    if "13" in attr_name or attr_name == "ean":
                        ean = ean or raw
                    gtin = gtin or raw
                    logger.info(f"[Strategy 2] UPC via meta {meta_attrs} | upc={upc}")
                    break

        # 3. Label/value extraction from visible page text
        if not upc:
            upc = _extract_upc(soup)

        # 4. If we still have no gtin/ean, try label/value scan for those fields
        #    Validate through _coerce_upc to reject non-numeric junk text.
        if not gtin:
            raw_gtin = _find_label_value(soup, ["gtin", "gtin-13", "gtin-12"])
            gtin = _coerce_upc(raw_gtin)
        if not ean:
            raw_ean = _find_label_value(soup, ["ean", "ean-13", "ean code"])
            ean = _coerce_upc(raw_ean)
    ...
    # ── Log what was found ────────────────────────────────────────────────────
    all_fields = [
        "product_name", "brand", "sku", "upc", "gtin", "ean",
