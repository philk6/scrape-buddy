...
from scraper import debug_scrape
from strategies import playwright_catalog
from strategies import playwright_scraper
...
def _run_scrape_worker(run_id: int, url: str, max_pages: int = 3) -> None:
    """
    Run the Playwright scrape pipeline + enrichment in a background thread.

    Pipeline:
      1. Open persistent Playwright browser
      2. Render listing page, collect product links (universal signals)
      3. Paginate up to max_pages, collecting ALL links first
      4. Visit every detail page and extract fields
      5. Enrich (pack parsing + UPC lookup)
      6. Save to database

    Calls database.complete_run() on success or database.fail_run() on error.
    """
    try:
        logging.info(f"[Job {run_id}] Starting Playwright pipeline (max_pages={max_pages})")
        result = playwright_scraper.run(url, max_pages=max_pages)
...
def scrape():
    ...
    The Playwright pipeline handles everything: rendering, link collection,
    pagination, and detail-page extraction.  No pre-fetch or page
    classification needed.

    Request body:
        { "url": "https://...", "label": "Optional label", "max_pages": 3 }
    ...
    # Launch the Playwright pipeline in the background
    threading.Thread(
        target=_run_scrape_worker,
        args=(run_id, url, max_pages),
        daemon=True,
